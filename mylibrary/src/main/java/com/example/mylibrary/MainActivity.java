...
import static com.example.mylibrary.ToasterMessage.editor;
...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    ...
    String accountKey,logo,token,configuration,safetyMessage,s3Bucket,api,firestoreDb,firestoreCollection,firestoreUIData,profilePicBucket,profilePicThumbnailsBucket;
    ...
    public void signInClick(){
        signIn.setOnClickListener(v -> {
            ...
            if (!utilitySelectedOne) {
                ...
                {
                    Toast.makeText(MainActivity.this, "Email and Phone fields can not be left blank!",
                            Toast.LENGTH_SHORT).show();
                    return;
                }

                if (email.getText().toString().isEmpty()) {
                    Toast.makeText(MainActivity.this, getResources().getString(R.string.empty_email),
                            Toast.LENGTH_SHORT).show();
                    return;
                } else {
...
// onClick of button perform this simplest code.
                    ...
                    {
                        Toast.makeText(MainActivity.this, getResources().getString(R.string.invalid_email),
                                Toast.LENGTH_SHORT).show();
                        return;
                    }
                ...
                if (phoneNo.isEmpty()) {
                    Toast.makeText(MainActivity.this, getResources().getString(R.string.empty_phone_number),
                            Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if (Global.currentLocation != null) {
                    ...
                    try {
                        ...
                        if (Global.isNetworkAvailable(MainActivity.this)) {
                            Global.createAndStartProgressBar(MainActivity.this);
                            STAsyncHttpConnection async = new STAsyncHttpConnection(MainActivity.this);
                            async.execute(Global.signUpURL+"/?email="+email.getText().toString().trim()+"&phone="+phoneNo);

                        } else {
                            Toast.makeText(MainActivity.this, getResources().getString(R.string.internet_error),
                                    Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception e) {
                        Log.e("log_tag",
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        Toast.makeText(MainActivity.this, getResources().getString(R.string.enable_location),
                                Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(MainActivity.this, getResources().getString(R.string.no_location),
                                Toast.LENGTH_SHORT).show();
                    }
                }
            ...
            else
            {
                ...
                if (Global.currentLocation != null) {
                    ...
                    try {
                        ...
                        if(utilitySelected) {
                            if(sharedPreferences.getBoolean("otpValidationRequired",false)) {
                                if (Global.isNetworkAvailable(MainActivity.this)) {
                                    Global.createAndStartProgressBar(MainActivity.this);
                                    HttpGetRequest async = new HttpGetRequest();
                                    async.execute(Global.getOTP+sharedPreferences.getString("phoneNumber",""));
                                } else {
                                    Toast.makeText(MainActivity.this, getResources().getString(R.string.internet_error),
                                            Toast.LENGTH_SHORT).show();
                                }
                            ...
                            {
                                if(sharedPreferences.getBoolean("adminApprovalRequired",false)) {
                                    if (!sharedPreferences.getString("subscriptionStatus", "").equals("approved")) {
                                        if (sharedPreferences.getString("userID", "").equals("0")) {
                                           /* Intent i = new Intent(MainActivity.this, MyProfile.class);
                                            ...
                                            if (!sharedPreferences.getString("subscriptionStatus", "").equals("")) {
                                              /*  Intent i = new Intent(MainActivity.this, AdminApprovalScreen.class);
                                                i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
                                    ...
                                    else
                                    {
                                        if (!sharedPreferences.getBoolean("termsAccepted", false)) {
                                          /*  Intent loginIntent = new Intent();
                                            ...
                                            loginIntent.setClass(MainActivity.this, TermsConditions.class);
                                            editor.putBoolean("ACCEPTANCE", sharedPreferences.getBoolean("termsAccepted", false));
                                            editor.putBoolean("userProfileCreated", true);
                                            editor.commit();
...
*/
                                          ...
                                          //  SharedPreferences.Editor editor = sharedPreferences.edit();
                                            ...
                                            try {
                                                ...
                                                jsonParam.put("version", MainActivity.this.getPackageManager().getPackageInfo(getPackageName(), 0).versionName);
                                                ...
                                                deviceParam.put("version", MainActivity.this.getPackageManager().getPackageInfo(getPackageName(), 0).versionName);
                                                deviceParam.put("pushNotificationToken", sharedPreferences.getString("pushNotificationToken",""));
                                            ...
                                            try {
                                                params.put("version", MainActivity.this.getPackageManager().getPackageInfo(getPackageName(), 0).versionName);
                                            } catch (Exception e) {

                                            }
                                            params.put("phone", sharedPreferences.getString("phoneNumber", ""));
                                            params.put("email", sharedPreferences.getString("emailAddress", ""));


                                        /*    if (Global.isNetworkAvailable(MainActivity.this)) {
                                                updateDeviceConfiguration(MainActivity.this, params);
                                            } else {
                                                Toast.makeText(MainActivity.this, getResources().getString(R.string.internet_error),
                                                        Toast.LENGTH_SHORT).show();
                                            }*
...
/*
                        ...
                        {
                            Toast.makeText(MainActivity.this, getResources().getString(R.string.select_utility),
                                    Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception e) {
                        Log.e("log_tag",
    ...
    public void processWebServiceResponse() throws JSONException {
        responseJSON = sharedPreferences.getString("fullResponseData", "");
        ...
        if(!responseData.getBoolean("Error")) {
            ...
            if(subscriptionsArray.length()==0)
            {
                ...
                adapter = new ArrayAdapter<String>(MainActivity.this, R.layout.spinner_item,
                        positions);
    ...
    private class DownloadImage extends AsyncTask<String, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(String... URL) {
            ...
            try {
                ...
                SaveImage(bitmap,sharedPreferences.getString("imageName",imageName));
    ...
    public class HttpGetRequest extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params){
            ...
            try {
                ...
                connection.setRequestProperty ("x-account-key", sharedPreferences.getString("accountKey",""));
                connection.setRequestProperty ("x-access-token", sharedPreferences.getString("token",""));
                connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
                connection.setRequestProperty("x-application", "PDM");
                connection.setRequestProperty("x-user", sharedPreferences.getString("phoneNumber",""));
        ...
        protected void onPostExecute(String result){
            ...
            try {
               ...
               /* Intent intent = new Intent();
                ...
                intent.setClass(MainActivity.this, EnterOTP.class);
                        ...
                        if(result!=null)
                        {
                            if(result.has("email")) {
                                if (result.getJSONArray("email").length() != 0) {
                                    ...
                                    MainActivity.email.setText(emailValue);
                                    MainActivity.email.setTextColor(Color.parseColor("#5A5A5A"));
                                }else
                                {
